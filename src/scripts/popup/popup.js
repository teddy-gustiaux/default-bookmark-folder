...
async function buildBookmarkFolderPopupItems(bookmarkItem, indent) {
...
async function buildBookmarkFolderPopupTree(bookmarkItems) {
	await buildBookmarkFolderPopupItems(bookmarkItems[0], 0);
}
...
async function saveBookmarkTo(info) {
	...
	const bookmarkTreeNode = {
		title: `${activeTab.title}${DBF_INTERNAL_INDICATOR}`,
		url: activeTab.url,
		parentId: folderId,
	};
...
async function popupOpened() {
	...
	await buildBookmarkFolderPopupTree(bookmarkTree);
...
document.addEventListener('DOMContentLoaded', Display.insertSvgIcons);
document.addEventListener('DOMContentLoaded', Display.insertDataFromLocales);
